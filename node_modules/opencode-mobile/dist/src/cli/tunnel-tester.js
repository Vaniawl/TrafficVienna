 ...
 */
export async function testTunnelProviders(port, options) {
    const results = [];
 ...
 * Test ngrok provider
 */
async function testNgrok(port, _options) {
    const results = [];
    const result = {
        provider: "ngrok",
        installed: false,
    ...
    if (configPath) {
        ...
        try {
            ...
            const hasAuthtoken = content.includes("authtoken:") || content.includes("agent:") && content.includes("authtoken:");
 ...
 */
async function testCloudflare(port, _options) {
    const results = [];
    const result = {
        provider: "cloudflare",
        installed: false,
 ...
 */
async function testLocaltunnel(port, _options) {
    const results = [];
    const result = {
...
export function compareTunnelProviders(results) {
    ...
    if (available.length === 0) {
        recommendations.push("No tunnel providers are working. Check your network connection and credentials.");
    }
    else if (available.length === 1) {
        recommendations.push(`Only ${available[0]} is available. Consider setting up additional providers for redundancy.`);
    }
    else {
        recommendations.push(`${available.join(", ")} are available. ${fastest.provider} is the fastest.`);
    }
    return recommendations;
}
//# sourceMappingURL=tunnel-tester.js.map